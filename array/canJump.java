...
class Solution {
    public int jump(int[] nums) {
        ...
        int currentEnd = 0;
        int furthest = 0;
        if(furthest >= nums.length-1) return ++jumps;
...
/*
        //precondition: fur <= nums.length-1

        //preservation: i+nums[i] = fur<= nums.length-1

        //postcondition: fur=nums.length-1=cur, i: 0:cur

        //iterate i, update cur, calculate fur;
 */
